...
# ✅ Cambio de ruta S3 a helena-datalake
COPY_MONTHLY_TRIPS_SQL = COPY_SQL.format(
    "trips",
    "s3://helena-datalake/data-pipelines/divvy/partitioned/{year}/{month}/divvy_trips.csv"
)

COPY_ALL_TRIPS_SQL = COPY_SQL.format(
    "trips",
    "s3://helena-datalake/data-pipelines/divvy/unpartitioned/divvy_trips_2018.csv"
)

COPY_STATIONS_SQL = COPY_SQL.format(
    "stations",
    "s3://helena-datalake/data-pipelines/divvy/unpartitioned/divvy_stations_2017.csv"
)

# ✅ Agregar sentencias para el tráfico de estaciones
